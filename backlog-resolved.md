...
- Verification: The focused library regression reproduced the callback identity change before the fix and passed after it. The full Jest suite passed with 35 suites and 143 tests; lint, TypeScript, and `git diff --check` passed. Release APK 0.9.14/versionCode 20 was signed with the existing release certificate, installed on `emulator-5554`, and tested with the preserved library data: the drawer opened, Library displayed the saved entries, CPU stayed low, and the first book opened the Reader with visible pages. The public `v0.9.14` manifest and APK/Android OTA/iOS OTA assets returned HTTP 200 with matching sizes and SHA-256 hashes. Starting from 0.9.13, Settings downloaded the 0.9.14 OTA, the app restarted into `Installed app code: 0.9.14`, and Library → Reader worked from the active OTA bundle without fatal errors. Captures and UI dumps are in `artifacts/release-0.9.14-*.png`, `artifacts/ota-0.9.14-*.png`, and matching XML/logcat files.
